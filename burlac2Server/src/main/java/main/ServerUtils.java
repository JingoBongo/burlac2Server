...
import static main.UdpMulticastServer.prevMessage;
...
public class ServerUtils {
    public static void processMessage(String str, DatagramSocket socket,  DatagramPacket packetToReceive,  String senderNickname) throws BadPaddingException, NoSuchAlgorithmException, IllegalBlockSizeException, NoSuchPaddingException, InvalidKeyException, IOException {
        ...
        switch (sw) {
            case 1:
                processCommand(str, socket, packetToReceive, senderNickname);
                break;
    ...
    private static void processText(String str, DatagramSocket socket,  String senderNickname) throws IllegalBlockSizeException, NoSuchAlgorithmException, IOException, BadPaddingException, NoSuchPaddingException, InvalidKeyException {
        ...
        for(User u : UdpMulticastServer.userList){
            if(u.getNickname().equals(senderNickname) || senderNickname.equals("resent")){
                if(u.isLogged()){
                    canSend = true;
                }
            }
    ...
    public static void processCommand(String str, DatagramSocket socket, DatagramPacket packetToReceive, String senderNickname) throws BadPaddingException, NoSuchAlgorithmException, IllegalBlockSizeException, NoSuchPaddingException, InvalidKeyException, IOException {
        //str = str.substring(1);
        ...
        if (arr.length > 1) {
            switch (arr[1]) {
                ...
                case "quit":
                    if(isAdmin){
                        UdpMulticastServer.whileLoop = false;
                    }
                    incorrectCommand = false|!isAdmin;
                    break;

                case "ls":
                    if(arr[2].equals("all")){
                        seeTheMatrix(socket, port, isAdmin);
                    }
                    incorrectCommand = false|!isAdmin;
                    break;

                case "kick":    // kick username
                    kick(socket, arr[2], isAdmin);
                    incorrectCommand = false|!isAdmin;
                    break;

                case "disconnect":
                    disconnect(port, socket);
                    incorrectCommand = false;
                    break;

                case "resend":
//                    System.err.println(prevMessage);
                    processText(prevMessage, socket, "resent");
                    incorrectCommand = false;
                break;
...
//!! change nickname newMickname
                ...
                case "help":
                    if(isAdmin){
                        sendMessage(socket, "SYSTEM: '!! login username password'\n" +
                                                    ...
                                                    "          '!! ls all'\n" +
                                                    "          '!! kick username'\n" +
                                                    "          '!! quit   . It would shut down the server.'\n" +
    ...
    private static void seeTheMatrix(DatagramSocket socket, int port, boolean isAdmin) throws IllegalBlockSizeException, NoSuchAlgorithmException, IOException, BadPaddingException, NoSuchPaddingException, InvalidKeyException {
        if(isAdmin){
            for(User u : UdpMulticastServer.userList) {
                if(u.isLogged()){
                    sendMessage(socket, "User: " + u.getUsername() + ". Nickname: " + u.getNickname() + ". Port: " + u.getIpPort(), port);
                }
            }
    ...
    private static void kick(DatagramSocket socket, String username, boolean isAdmin) throws IllegalBlockSizeException, NoSuchAlgorithmException, IOException, BadPaddingException, NoSuchPaddingException, InvalidKeyException {

        if(isAdmin) {
            for (User u : UdpMulticastServer.userList) {
                if (u.getUsername().equals(username)) {
                    sendMessage(socket, "SYSTEM: finita la commedia for " + u.getUsername() + ". Successfully kicked in the knee ", u.getIpPort());
                    u.setIpPort(0);
                    u.setLogged(false);
                }
                if (u.getUsername().equals("admin")) {
                    sendMessage(socket, "SYSTEM: you kicked this bastard: " + username + ", sheriff.", u.getIpPort());
                }
            }
        }
    }

    private static void disconnect(int port, DatagramSocket socket) throws IllegalBlockSizeException, NoSuchAlgorithmException, IOException, BadPaddingException, NoSuchPaddingException, InvalidKeyException {
        for(User u : UdpMulticastServer.userList){
            if(u.getIpPort() == port){
                sendMessage(socket,  "SYSTEM: finita la commedia for " + u.getUsername() + ". Successfully logged out", port);
                u.setIpPort(0);
                u.setLogged(false);
            }
        }
        ...
        for (User u : UdpMulticastServer.userList) {
            if (u.getUsername().equals(username) && u.getPassword().equals(password)) {
                ...
                if (!u.isLogged()) {
                    u.setIpPort(ipPort);
                    u.setLogged(true);
                    sendMessage(socket,  "SYSTEM: " + u.getUsername() + " successfully logged in", ipPort);
                } else {
